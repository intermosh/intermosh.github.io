...
async function computeDensity({ requestId, segments, fullText }) {
  ...
  // Transiciones entre segmentos consecutivos
  const transitionSims = [];
  for (let i = 1; i < segEmbs.length; i++) {
    transitionSims.push(dot(segEmbs[i - 1], segEmbs[i]));
  }
  // Umbral adaptativo: media de transiciones - 1σ.
  // e5-small produce cosenos altos (0.7-0.95) para español — umbral fijo no funciona.
  // [Adaptativo, similar a DETECT_GAPS: media−σ de la distribución]
  const avgTransition = transitionSims.length
    ? transitionSims.reduce((a, b) => a + b, 0) / transitionSims.length : 0.8;
  const transStd = transitionSims.length > 1
    ? Math.sqrt(transitionSims.reduce((s, v) => s + (v - avgTransition) ** 2, 0) / transitionSims.length)
    : 0.1;
  const adaptiveThreshold = avgTransition - transStd;

  const changes = [];
  for (let i = 0; i < transitionSims.length; i++) {
    if (transitionSims[i] < adaptiveThreshold) {
      changes.push({ afterSegment: i + 1, similarity: transitionSims[i] });
    }
  }

  // topicsPerMinute: cambios detectados / minutos estimados.
  // segments.length ≈ minutos (cada segmento = 1 min de habla a WPM).
  const estMinutes = segments.length || 1;
  const topicsPerMinute = changes.length / estMinutes;
