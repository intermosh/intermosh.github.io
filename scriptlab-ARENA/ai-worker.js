...
async function computeDensity({ requestId, segments, fullText }) {
  ...
  // Cambios temáticos: similitud entre segmentos consecutivos por debajo del promedio.
  // Umbral: avgGlobalSim - 0.15 (delta empírico, [NO VALIDADO]).
  const changes = [];
  for (let i = 1; i < segEmbs.length; i++) {
    const sim = dot(segEmbs[i - 1], segEmbs[i]);
    if (sim < avgGlobalSim - 0.15) {
      changes.push({ afterSegment: i - 1, similarity: sim });
    }
  }

  const topicsPerMinute = segments.length > 0 ? changes.length / segments.length : 0;
