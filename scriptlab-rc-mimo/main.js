...
import {
  ...
  renderExtractive, renderRedundancy, renderDensity, renderGaps, refreshSemanticCards
} from './render.js';
import { exportJSON, exportMarkdown, exportHTML, exportTXT, exportPDF, exportDiagnostics, importProject } from './export-import.js';
...
function bind() {
  ...
  $$('[data-export]').forEach(btn => btn.onclick = async () => {
    ...
    else if (kind === 'html') exportHTML(state.p, a, state.retentionResult, state.sentimentResult, state.aiResult);
    else if (kind === 'txt') exportTXT(state.p);
    else if (kind === 'pdf') exportPDF(state.p, a, state.retentionResult, state.sentimentResult, state.aiResult);
    else exportJSON(state.p, a, c);
    $('#export-menu').hidden = true;
  });

  $$('[data-export-diag]').forEach(btn => btn.onclick = async () => {
    const a = analysis();
    const format = btn.dataset.exportDiag;
    exportDiagnostics(state.p, a, state.retentionResult, state.sentimentResult, state.aiResult, state.deepResult, format);
    $('#export-menu').hidden = true;
  });
  ...
  $('#mode-ai')?.addEventListener('click', async () => {
    // Seleccionar modo IA inmediatamente para feedback visual
    state.mode = 'ia';
    ...
    render();
    await put('projects', state.p);
    // Verificar modelos y cargar si es necesario
    state.modelsReady = await modelsAreReady();
    if (state.modelsReady) {
      await syncWorkerWithState();
    }
    // Si los modelos no están listos, el usuario ve la UI de "descargá modelos"
    // pero el modo ya queda seleccionado como IA.
    paintMode();
    refreshAIActivityPill();
    render();
  });
...
async function runDeep() {
  ...
  if (sentences.length >= 3) {
    promises.push(
      workerSend('EXTRACT_KEY_SENTENCES', { sentences, fullText, topN: 5 })
        ...
        .catch(err => { state._extractiveResult = { error: err.message }; })
    );
  }
  ...
  if (eligible.length >= 2) {
    promises.push(
      workerSend('COMPUTE_REDUNDANCY', {
        ...
        .catch(err => { state._redundancyResult = { error: err.message }; })
    );
  }
  ...
  if (fullText.trim()) {
    ...
    promises.push(
      workerSend('COMPUTE_DENSITY', { segments, fullText })
        ...
        .catch(err => { state._densityResult = { error: err.message }; })
    );
  }
  ...
  promises.push(
    Promise.resolve()
      .then(() => renderGaps())
      .catch(err => { state._gapsResult = { error: err.message }; })
  );
